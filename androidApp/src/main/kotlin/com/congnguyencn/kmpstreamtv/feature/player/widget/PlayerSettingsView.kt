...
internal class PlayerSettingsView @JvmOverloads constructor(
    context: Context,
    attrs: AttributeSet? = null,
    defStyleAttr: Int = 0,
) : FrameLayout(context, attrs, defStyleAttr) {
    private val columns = LinearLayout(context).apply {
    ...
    init {
        ...
        addView(columns, LayoutParams(MATCH_PARENT, MATCH_PARENT).apply {
    ...
    fun render(state: StreamTvPlayerState, speed: Float) {
        latestState = state
        selectedSpeed = speed
    ...
    private fun createColumn(type: PlayerSettingType, options: List<PlayerSettingOption>) =
        LinearLayout(context).apply {
            ...
            addView(
                RecyclerView(context).apply {
                    ...
                    adapter = PlayerSettingAdapter(options) { option ->
    ...
    private fun optionsFor(type: PlayerSettingType): List<PlayerSettingOption> = when (type) {
        PlayerSettingType.SPEED -> SPEEDS.map { speed ->
            PlayerSettingOption(
        ...
        PlayerSettingType.VIDEO -> {
            ...
            listOf(
                ...
                val label = buildString {
                    append(if (track.height > 0) "${track.height}p" else context.getString(R.string.player_setting_unknown))
        ...
        PlayerSettingType.AUDIO -> latestState.audioTracks.map { track ->
            PlayerSettingOption(track.displayLabel(), track.id, track.isSelected)
        }

        PlayerSettingType.SUBTITLE -> listOf(
            PlayerSettingOption(
            ...
            PlayerSettingOption(track.label.ifBlank { track.language.displayLanguage() }, track.id, track.isSelected)
        }
    }
    ...
    private fun Float.displaySpeed(): String =
        if (this == toInt().toFloat()) "${toInt()}x" else "${this}x"
...
internal enum class PlayerSettingType(@StringRes val titleRes: Int) {
...
private class PlayerSettingAdapter(
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): OptionViewHolder =
        OptionViewHolder(TextView(parent.context).apply {
    ...
    class OptionViewHolder(private val textView: TextView) : RecyclerView.ViewHolder(textView) {
        fun bind(item: PlayerSettingOption, onClick: (PlayerSettingOption) -> Unit) = with(textView) {
            text = item.title
            ...
            setTextAppearance(
                if (item.selected) R.style.MediaPlayerActionSettingActiveTextAppearance
                else R.style.MediaPlayerActionSettingInActiveTextAppearance,
            )
