...
internal class HomeUiMapper {
    fun map(sections: List<HomeSection>): List<HomeSectionUiModel> = sections.map { section ->
        HomeSectionUiModel(
            id = section.id,
            title = section.title,
...
private fun HomeSectionViewType.toPresentation(): HomeSectionPresentation = when (this) {
...
private fun Content.toUiModel() = HomeContentUiModel(
    id = id,
    videoUrl = videoUrl,
